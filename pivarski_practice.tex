...
\vspace{0.1 cm}
\hfill \includegraphics[width=0.8\linewidth]{matrix_description_onestation.png}
...
\item 33,000 events from a \textcolor{darkblue}{9-minute} long run \mbox{(3/4 of 2008 beam data)\hspace{-2 cm}}
\end{itemize}
\end{itemize}
\end{minipage}}
...
\begin{frame}
\frametitle{Sample fit results: \only<1>{MC}\only<2>{data}}
...
\begin{itemize}
\item Projection of \textcolor{red}{fits (all parameters = 0 other than the one shown)} overlaid on {\it \only<1>{simulated}\only<2>{real}} data (profile plots)
\item \only<1>{All of the above is for one chamber; the rest are similar}\only<2>{This is the same chamber in real data}
\end{itemize}
\end{frame}
